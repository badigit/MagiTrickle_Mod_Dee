...
.PHONY: all clear clean download download_backend download_frontend redownload redownload_backend redownload_frontend build build_backend build_frontend rebuild rebuild_backend rebuild_frontend prepare_files package package_ipk package_apk FORCE
...
rebuild_backend:
	@rm -f "$(STAMPS_DIR)/build-backend-$(UNIQUE_NAME)"
