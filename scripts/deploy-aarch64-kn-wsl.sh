...
HOST_ALIAS="${ROUTER_SSH:?не задан ROUTER_SSH (ssh-алиас или user@адрес): env или .router.env}"
SSH_PORT="${ROUTER_PORT:-222}"
REMOTE_TMP_DIR="/opt/root/tmp"

shopt -s nullglob
candidates=("$ROOT_DIR"/.build/magitrickle_*_entware_aarch64-3.10_kn.ipk)
shopt -u nullglob

if [[ ${#candidates[@]} -eq 0 ]]; then
  echo "No aarch64-3.10_kn package found in \"$ROOT_DIR/.build\"." >&2
  exit 1
fi
...
ssh -p "$SSH_PORT" "$HOST_ALIAS" "opkg install --force-reinstall \"$REMOTE_PACKAGE_PATH\" && /opt/etc/init.d/S99magitrickle status"
